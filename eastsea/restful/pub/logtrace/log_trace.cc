...
#
#    Filename     : /home/tianyiheng_test/runner/northsea/restful/plugins/news/log_trace.cc
#    Author       : tianyiheng
...
void LogTrace::Init(std::string sockfile){
  ...
  int connect_ret = ::connect(sock_fd_, (struct sockaddr*)&addr, sizeof(addr));
	LOG_DEBUG2("%s", sockfile.c_str());
  if (0 > connect_ret){                                            
    LOG_DEBUG2("[connect error!]:%s:%d\n", sockfile.c_str(), errno);
