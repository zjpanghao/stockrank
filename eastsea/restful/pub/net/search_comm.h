 ...
 public:
  ...
  inline void init() {
    ...
    if (false == r) {
      ...
      std::transform\
      (message_.begin(), message_.end(), message_.begin(), toupper);
    }
  }
...
class SendSearch:public SendPacketBase {
 public:
  SendSearch() {
    ...
    event_.reset( new base_logic::ListValue() );
  }

  NetBase* release() {
    head_->Set("stock", name_.release());
    head_->Set("hy", hy_.release());
    head_->Set("gn", gn_.release());
    head_->Set("event", event_.release());
    this->set_status(1);
  ...
  inline void set_event(base_logic::Value* value) {
    event_->Append(value);
  }
 ...
 private:
  ...
  scoped_ptr<base_logic::ListValue> event_;
};
